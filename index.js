...
async function run() {
  try {
    ...
    app.get("/admin-stats", verifyToken, verifyAdmin, async (req, res) => {
      ...
      // const revenue = payments.reduce((acc, payment) => acc + payment.price, 0);
     ...
     * 3. for every item in the menu collection that you found from a payment entry (document), do something with the price or category
     */
    ...
    app.get("/order-stats", verifyToken, verifyAdmin, async (req, res) => {
      const result = await paymentCollection
        .aggregate([
          {
            $unwind: "$menuIds", // Split the array into separate documents
          },
          {
            $lookup: {
              from: "menu",
